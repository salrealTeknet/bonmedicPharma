    ...
    def post(self, request):
        ...
        if form.is_valid() and formset.is_valid():
            try:
                ...
                messages.error(request, f"Error occurred: {exc}")
                context = {
                    'form': form,
                    'formset': formset,
                    'stocks': Stock.objects.filter(is_deleted=False)
...
class PurchaseBillView(View):
    ...
    def get(self, request, billno):
        ...
        billdetails = PurchaseBillDetails.objects.filter(billno=billno).first()
    ...
    def post(self, request, billno):
        ...
        if form.is_valid():
            billdetailsobj = PurchaseBillDetails.objects.filter(billno=billno).first()
            if billdetailsobj:
                ...
                messages.error(request, "Bill details not found")
